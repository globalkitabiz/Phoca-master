# Phoca (Vocapic) 프로젝트 정보

> **AI 기반 영어 어휘 학습 서비스** - 사진으로 단어를 배우는 교육 플랫폼

---

## 배포 아키텍처

```
┌─────────────────────────────────────────────────────────────────────┐
│                        사용자 브라우저                                │
└─────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────┐
│                    Cloudflare Pages (Frontend)                      │
│                 https://phoca-master.pages.dev                      │
│                          Next.js 12                                 │
└─────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────┐
│                     Railway (Backend API)                           │
│           https://phoca-master-production.up.railway.app            │
│                         NestJS + TypeScript                         │
└─────────────────────────────────────────────────────────────────────┘
           │                    │                    │
           ▼                    ▼                    ▼
┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
│   Supabase      │  │  Cloudflare R2  │  │   Replicate     │
│   (Database)    │  │  (Image Storage)│  │   (AI API)      │
│   PostgreSQL    │  │   S3 호환       │  │   객체인식/그림  │
└─────────────────┘  └─────────────────┘  └─────────────────┘
                                                    │
                                                    ▼
                                          ┌─────────────────┐
                                          │  Google Cloud   │
                                          │  Translation    │
                                          │   (번역 API)    │
                                          └─────────────────┘
```

---

## 서비스별 역할 요약

| 서비스 | 역할 | 설명 |
|--------|------|------|
| **Cloudflare Pages** | 프론트엔드 호스팅 | Next.js 웹앱을 전 세계 CDN으로 배포 |
| **Railway** | 백엔드 API 서버 | NestJS 서버로 인증, CRUD, AI 호출 등 모든 백엔드 로직 처리 |
| **Supabase** | 데이터베이스 | PostgreSQL DB로 사용자, 단어장, 단어, 북마크, 퀴즈 기록 저장 |
| **Cloudflare R2** | 이미지 저장소 | S3 호환 스토리지로 사용자 업로드 이미지, AI 인식 이미지 저장 |
| **Replicate** | AI 모델 API | 이미지에서 객체 인식(YOLO), 그림 분석(CLIP) |
| **Google Translate** | 번역 API | 인식된 영어 단어를 한국어로 자동 번역 |

### 데이터 흐름 (사진으로 단어장 만들기)
```
사용자 사진 업로드 → Cloudflare R2에 저장
                  → Replicate AI가 객체 인식
                  → Google Translate로 번역
                  → Supabase DB에 단어 저장
```

---

## 서비스별 상세 정보

### 1. Cloudflare Pages (Frontend)
| 항목 | 내용 |
|------|------|
| **역할** | 프론트엔드 웹 애플리케이션 호스팅 |
| **기술** | Next.js 12, React 18, TypeScript |
| **URL** | `https://phoca-master.pages.dev` (배포 완료 시) |
| **비용** | 무료 (무제한 요청) |
| **기능** | - 정적 사이트 호스팅<br>- CDN 글로벌 배포<br>- HTTPS 자동 제공 |

### 2. Railway (Backend API)
| 항목 | 내용 |
|------|------|
| **역할** | 백엔드 REST API 서버 |
| **기술** | NestJS, TypeScript, TypeORM |
| **URL** | `https://phoca-master-production.up.railway.app` |
| **비용** | 무료~$5/월 (500시간/월 무료) |
| **기능** | - 사용자 인증 (JWT)<br>- 단어/단어장 CRUD<br>- 이미지 업로드 처리<br>- AI API 호출<br>- 이메일 발송 |

### 3. Supabase (Database)
| 항목 | 내용 |
|------|------|
| **역할** | PostgreSQL 데이터베이스 |
| **Host** | `db.rdtvutjqtygrusulkfok.supabase.co` |
| **비용** | 무료 (500MB DB, 1GB Storage) |
| **저장 데이터** | - 사용자 정보 (Users)<br>- 단어장 (Wordbook)<br>- 단어 (Word)<br>- 북마크 (Bookmark)<br>- 퀴즈 기록 (Quiz) |

### 4. Cloudflare R2 (Image Storage)
| 항목 | 내용 |
|------|------|
| **역할** | 이미지 파일 저장소 (S3 호환) |
| **버킷** | `vocapic-images` |
| **Public URL** | `https://pub-2b336b019958442583369863e931a947.r2.dev` |
| **비용** | 무료 (10GB/월) |
| **저장 파일** | - 사용자 업로드 이미지<br>- AI 인식된 객체 이미지<br>- 프로필 이미지 |

### 5. Replicate (AI API)
| 항목 | 내용 |
|------|------|
| **역할** | AI 모델 API 제공 |
| **API Token** | `r8_TMoOp...` (일부 가림) |
| **비용** | 무료 크레딧 $5 → 종량제 |
| **기능** | - **객체 인식**: 이미지에서 물체 감지 (YOLO/DETR)<br>- **그림 인식**: 사용자가 그린 그림 분류 (CLIP) |

### 6. Google Cloud Translation
| 항목 | 내용 |
|------|------|
| **역할** | 영어→한국어 자동 번역 |
| **Project ID** | `vocapic-480015` |
| **비용** | 무료 (50만 글자/월) |
| **기능** | - 감지된 영어 단어를 한국어로 번역<br>- Cloud Translation API v2 |

---

## 환경 변수 정리

### Backend (Railway)
```env
# Database (Supabase)
DB_HOST=db.xxxxxxxxxxxx.supabase.co
DB_PORT=5432
DB_USERNAME=postgres
DB_PASSWORD=your-password
DB_NAME=postgres

# JWT
JWT_SECRET_KEY=your-super-secret-jwt-key-minimum-32-characters
JWT_EXPIRESIN=7d

# Cloudflare R2
R2_ACCESS_KEY_ID=your-r2-access-key
R2_SECRET_ACCESS_KEY=your-r2-secret-key
R2_ENDPOINT=https://your-account-id.r2.cloudflarestorage.com
R2_BUCKET_NAME=your-bucket-name
R2_PUBLIC_URL=https://pub-xxxxx.r2.dev

# Replicate AI
REPLICATE_API_TOKEN=r8_xxxxxxxxxxxx

# Google Translate
client_email=your-service-account@project-id.iam.gserviceaccount.com
private_key="-----BEGIN PRIVATE KEY-----\n...\n-----END PRIVATE KEY-----\n"
projectId=your-project-id

# Email (선택)
EMAIL_HOST=smtp.gmail.com
EMAIL_PORT=587
EMAIL_USER=your-email@gmail.com
EMAIL_PASS=your-app-password
```

### Frontend (Cloudflare Pages)
```env
NEXT_PUBLIC_SERVER_URL=https://phoca-master-production.up.railway.app
NEXT_PUBLIC_IMAGE_URL=https://pub-2b336b019958442583369863e931a947.r2.dev/
```

---

## 주요 기능

### 1. 단어장 만들기
1. 사용자가 사진 업로드
2. Replicate AI가 객체 인식
3. Google Translate로 한국어 번역
4. 단어장에 저장 (Supabase)
5. 이미지는 R2에 저장

### 2. 카드 매칭 게임
- 영어 단어와 이미지를 매칭하는 메모리 게임
- 단어장에서 8개 단어 랜덤 선택

### 3. 그림 그리기 퀴즈
1. 랜덤 영어 단어 제시
2. 사용자가 그림 그리기
3. Replicate AI가 그림 분석
4. 정답 여부 판정

### 4. 공개 단어장 탐색
- 다른 사용자의 공개 단어장 검색
- 북마크 기능

---

## 배포 상태

| 서비스 | 상태 | URL |
|--------|------|-----|
| Supabase DB | ✅ 완료 | `db.rdtvutjqtygrusulkfok.supabase.co` |
| Cloudflare R2 | ✅ 완료 | `https://pub-2b336b019958442583369863e931a947.r2.dev` |
| Replicate AI | ✅ 완료 | API 토큰 설정 완료 |
| Google Translate | ✅ 완료 | 서비스 계정 생성 완료 |
| Railway Backend | ✅ 완료 | `https://phoca-master-production.up.railway.app` |
| Cloudflare Pages | 🔄 진행중 | 빌드 설정 조정 필요 |

---

## 예상 월 비용

| 서비스 | 무료 한도 | 예상 비용 |
|--------|----------|----------|
| Supabase | 500MB DB | 무료 |
| Cloudflare R2 | 10GB Storage | 무료 |
| Cloudflare Pages | 무제한 | 무료 |
| Railway | 500시간/월 | 무료~$5 |
| Replicate | $5 크레딧 | 종량제 |
| Google Translate | 50만 글자 | 무료 |
| **총합** | - | **무료 ~ $5/월** |

---

## GitHub Repository

**URL**: https://github.com/globalkitabiz/Phoca-master

### 브랜치
- `main`: 배포용 메인 브랜치

### 주요 커밋
1. Initial commit: Vocapic with Cloudflare R2
2. Add nixpacks.toml for Railway deployment
3. Fix TypeScript errors
4. Configure Next.js for Cloudflare Pages

---

## 로컬 개발 환경 설정

### Backend
```bash
cd back
cp .env.example .dev.env
# .dev.env 파일에 환경 변수 설정
npm install
npm run start:dev
```

### Frontend
```bash
cd front
cp .env.example .env.local
# .env.local 파일에 환경 변수 설정
npm install
npm run dev
```

---

## 문제 해결 가이드

### Backend 500 에러
1. Railway 로그 확인
2. 환경 변수 누락 확인
3. DB 연결 확인

### 이미지 업로드 실패
1. R2 API 토큰 권한 확인
2. CORS 설정 확인
3. R2 Public URL 확인

### AI 인식 실패
1. Replicate API 토큰 확인
2. 크레딧 잔액 확인

### 번역 실패
1. Google Cloud 서비스 계정 확인
2. Translation API 활성화 확인
3. private_key 형식 확인 (줄바꿈 `\n` 포함)

---

*마지막 업데이트: 2025-12-03*
