...
export class WordController {
  constructor(
    ...
    private readonly aiMiddleware: AiMiddleware,
  ...
  @ApiOperation({
    summary: "단어 저장 API",
    description: "이미지를 입력받아 AI로 객체를 인식하고 단어 데이터를 생성한다.",
  })
  ...
  async uploadWord(@UploadedFile() file: Express.Multer.File) {
    ...
    // 2. Replicate AI로 객체 인식
    let wordEng: string[] = await this.aiMiddleware.detectObjects(wordKey);
    ...
    // 중복 제거
    wordEng = [...new Set(wordEng)];

    // 3. 영어 단어를 한국어로 번역
    const wordKor: string[] = [];
